    ...
    public class MovieService : IMovieService
    {
        ...
        public Movie GetById(int id)
        {
            ...
            if (movie != null)
            {
                ...
                movie.GenreNames = string.Join(',', genres);
            }
            return movie;
        }
        ...
        public MovieListVm List(string term="",bool paging=false, int currentPage=0)
        {
            ...
            {
                term = term.ToLower();
                var searchWords = term.Split(' '); // Split the search term into individual words
                list = list.Where(movie => searchWords.Any(word =>
                            movie.Title.ToLower().Contains(word) || 
                            movie.ReleaseYear.ToLower().Contains(word) || movie.Cast.ToLower().Contains(word) || movie.Director.ToLower().Contains(word)
